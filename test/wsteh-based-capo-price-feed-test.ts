...
export async function makeWstETHCAPO() {
    ...
    const oracle = await OracleFac.deploy(mgr.address, MAINNET_CONTRACTS.ETH_USD, MAINNET_CONTRACTS.WSTETH, "wstETH CAPO", FEED_DECIMALS, 3600, {
...
describe("wstETH CAPO price feed", () => {
    it("constructor: zero manager revert", async () => {
        ...
        const now = (await ethers.provider.getBlock("latest"))!.timestamp;
        await expect(
            Oracle.deploy(AddressZero, MAINNET_CONTRACTS.ETH_USD, MAINNET_CONTRACTS.WSTETH, "bad", 8, 0, {
