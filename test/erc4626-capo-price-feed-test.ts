...
export async function makeCAPOPriceFeed({
    ...
    const SimpleERC4626RatePriceFeed = await ethers.getContractFactory("SimpleERC4626RatePriceFeed");
    ...
    const ERC4626CorrelatedAssetsPriceOracleFactory = await ethers.getContractFactory("ERC4626CorrelatedAssetsPriceOracle");
...
describe("CAPO price feed", function () {
    it("reverts if constructed with bad manager address", async () => {
        ...
        const ERC4626CorrelatedAssetsPriceOracle = await ethers.getContractFactory("ERC4626CorrelatedAssetsPriceOracle");
    ...
    it("reverts if constructed with bad price feed", async () => {
        ...
        await expect(
            ERC4626CorrelatedAssetsPriceOracle.deploy(signer.address, AddressZero, await PriceFeedB.getAddress(), "CAPO Price Feed", 8, 0, {
                snapshotRatio: 0,
                snapshotTimestamp: 1,
                maxYearlyRatioGrowthPercent: 0
            })
        ...
        await expect(
            ERC4626CorrelatedAssetsPriceOracle.deploy(signer.address, await PriceFeedA.getAddress(), AddressZero, "CAPO Price Feed", 8, 0, {
                snapshotRatio: 0,
                snapshotTimestamp: 1,
                maxYearlyRatioGrowthPercent: 0
            })
    ...
    it("reverts if set cap parameters not by manager", async () => {
        ...
        await expect(
            CapoPriceFeed.connect(signer).updateSnapshot({
                snapshotRatio: 0,
                snapshotTimestamp: 0,
                maxYearlyRatioGrowthPercent: 0
            })
